...
use parse::{ParseError, StructTermElem, parse_maybe_struct_term_elem, parse_struct_term_elem,
                        pprint_struct_term_elem,
                        Expr, parse_expr, pprint_expr};

pub struct StructTerm {
    pub head_elems: Vec<StructTermElem>,
    pub tail: Option<Expr>,
    pub span: Span,
}

pub fn parse_struct_term_inner(tokens: &[Token]) -> Result<(Vec<StructTermElem>, Option<Expr>), ParseError> {
    ...
    for (i, token) in tokens.iter().enumerate() {
        ...
        match token.kind {
            ...
            TokenKind::Comma => {
                ...
                let head_elem = try!(parse_struct_term_elem(head_tokens));
            ...
            TokenKind::Semicolon => {
                ...
                let head_elem = try!(parse_maybe_struct_term_elem(head_tokens));
    ...
    let head_elem = try!(parse_maybe_struct_term_elem(tokens));
...
pub fn pprint_struct_term(struct_term: &StructTerm, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    if let Some((first, rest)) = struct_term.head_elems[..].split_first() {
        try!(pprint_struct_term_elem(first, f));
        for elem in rest {
            try!(write!(f, ", "));
            try!(pprint_struct_term_elem(elem, f));
        }
    }
